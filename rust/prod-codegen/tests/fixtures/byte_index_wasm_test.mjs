...
for(const size of [0,1,2,3,4,16,63,64]){
  for(let octet=0;octet<=255;octet++){
    ...
    const expected=size<4?255:input[3]===128?1:0;
    assert.equal(invoke(input),expected);
    passed++;
  }
}
assert.throws(()=>invoke(new Uint8Array(65)),WebAssembly.RuntimeError);
console.log(`PASS ${passed} actual import-free Wasm index cases and allocation rejection`);
