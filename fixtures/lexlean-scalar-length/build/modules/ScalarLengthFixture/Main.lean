...
namespace ScalarLengthFixture.Main
...
@[expose] public def scalarLength (value : String) : Nat := (LexLeanRuntime.length (value) : Nat)

@[expose] public def byteLength (value : String) : Nat := (LexLeanRuntime.length ((LexLeanRuntime.utf8Encode (value) : ByteArray)) : Nat)

@[expose] public def nestedLength (text : Text) : Nat := (LexLeanRuntime.length ((text).value) : Nat)

@[expose] public def entry (input : ByteArray) : ByteArray := (match (LexLeanRuntime.utf8Decode (input) : Option (String)) with | Option.none => ByteArray.mk #[255] | Option.some text => (match (Nat.beq (scalarLength (text)) (2)) with | Bool.false => ByteArray.mk #[0] | Bool.true => ByteArray.mk #[1]))

end ScalarLengthFixture.Main
