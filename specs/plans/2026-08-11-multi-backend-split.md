...
- Generated artifacts are never hand-edited. `rust/prod-core/kernel.ir`, `goldens.ir`, `roots.json`, `coverage.md`, `subset.json` are gitignored — never add them by name. **Do NOT use `git add -A` either** — it swept an unrelated, machine-local `skills.lock` containing auth tokens into this branch's first commit, and removing it needed a history rewrite. Add the paths you touched, explicitly. `lean/Conformance/golden.ir`, `golden-rejected.ir` and `specs/lean-for-production.md` are committed but regenerated only by the tooling.
