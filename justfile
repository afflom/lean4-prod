...
ci: fixture-provenance prod fmt-check lint wasm-check portable-package specialized-index typed-decimal core-wasm view text-view workspace-view wasm-sdk-fixture uor-fixture

# Exact imported byte-index specialization, generated source and execution.
specialized-index:
    bash scripts/check-specialized-index.sh
...
portable-package:
    scripts/check-portable-package.sh

# Export a real LexLean-generated byte dispatcher, build the import-free
# Hologram guest twice, inspect its ABI, and execute it in two engines.
core-wasm:
    scripts/check-core-wasm.sh
...
lean-fixtures:
    cd lean && lake build ProofFixtures PrismPMGeneratedFixture
