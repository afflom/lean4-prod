...
const fixture = join(root, 'fixtures/lexlean-scalar-length');
...
assert.equal(receipt.spec, 'lean4-prod/lexlean-scalar-length-fixture/1');
...
assert.deepEqual(manifest.modules, [{lean_module: 'ScalarLengthFixture.Main', module: 'Main', source_path: 'src/Main.lex.tex'}]);
...
assert.equal(receipt.generated_path, 'lean/Conformance/LexLeanScalarLength.lean');
const generated = read(join(root, receipt.generated_path));
assert.equal(hash(generated), receipt.generated_sha256);
assert.deepEqual(generated, read(join(fixture, 'build/modules/ScalarLengthFixture/Main.lean')));
assert.ok(!/\b(?:sorry|admit|axiom|opaque)\b/.test(generated.toString('utf8')));
console.log('scalar length fixture source, compiler identity, generated module and complete output binding passed');
