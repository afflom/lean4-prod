...
- **`div_or_mod` (`lib.rs:1556`) — division by zero returns the DIVIDEND, not zero.** `Init/Prelude.lean:2183`: *"When the divisor is `0`, the result is the dividend rather than an error"*, doctest `5 % 0 = 5`; `Int.emod_zero : a % 0 = a`. The current code carries a `zero_is_dividend` flag distinguishing `Div` from `Mod`. Rendering `0` for both was a real defect shipped since M3.
